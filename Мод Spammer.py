# spymer modiffied
# Автор Мода: https://vk.com/id.artemhasanow
import os
def MAIN():
	try:
		import requests
		...
		def Main():
				...
				try:
					if int(phone):
						...
						try:
							...
							info="Неверно введено кол-во кругов"
				except:
					info="Неверно введен номер телефона"
			...
			def n_send(phone, count, proxies):
				...
				while iteration < count:
					...
					sms()
					iteration+=1
					print("{}".format(phone)+": круг {} пройден.".format(iteration))
				print("\nСпам на {} закончен. Кол-во кругов {}".format(phone, count))
				exit()
		...
		os.system("pip install requests proxyscrape")
